...
const WORKING = { start: 540, end: 1020 }; // 09:00 - 17:00
const MORNING   = { start: 540,  end: 780  }; // 09:00 - 13:00
const AFTERNOON = { start: 780,  end: 1020 }; // 13:00 - 17:00
    ...
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh", background: "#0b0b14", padding: 20 }}>
      <div style={{ width: "100%", maxWidth: 380, padding: 32, background: "#13131f", border: "1px solid #1a1a30", borderRadius: 24, boxShadow: "0 24px 64px #000a", transform: shake ? "translateX(-6px)" : "none", transition: "transform 0.1s" }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <div style={{ fontSize: 11, color: "#7c3aed", fontWeight: 700, letterSpacing: 3, marginTop: 3 }}>CONSULTING</div>
        </div>
        {step === "creds" ? (
          <>
          ...
          <>
            ...
            <button onClick={() => { setStep("creds"); setPin(""); setError(""); }} style={{ width: "100%", background: "transparent", color: "#475569", border: "none", fontSize: 13, cursor: "pointer", padding: "10px" }}>← Back</button>
          </>
        )}
      </div>
    </div>
...
function Dashboard({ onLogout }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", height: isMobile ? "auto" : "100vh", minHeight: "100vh", background: "#0b0b14", color: "#e2e8f0", fontFamily: "system-ui,sans-serif", overflow: isMobile ? "visible" : "hidden" }}>
        ...
        <div style={{ position: "fixed", inset: 0, background: "#000c", zIndex: 100, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={() => setDetail(null)}>
          <div style={{ background: "#13131f", border: "1px solid #2a2a45", borderRadius: "20px 20px 0 0", padding: 24, width: "100%", maxWidth: 500, boxShadow: "0 -8px 40px #000a", maxHeight: "85vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <span style={{ fontWeight: 800, fontSize: 17 }}>Booking — {detail.res_id}</span>
      ...
      {/* DESKTOP Sidebar */}
      {!isMobile && (
        <div style={{ width: 210, background: "#0f0f1c", borderRight: "1px solid #1a1a30", display: "flex", flexDirection: "column", flexShrink: 0 }}>
          <div style={{ padding: "20px 16px", borderBottom: "1px solid #1a1a30" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
              ...
              <div><div style={{ fontWeight: 800, fontSize: 13, color: "#fff" }}>SignalCraft</div><div style={{ fontSize: 9, color: "#7c3aed", fontWeight: 700, letterSpacing: 2 }}>CONSULTING</div></div>
            </div>
          </div>
        ...
        <div style={{ flex: isMobile ? undefined : 1, overflow: isMobile ? "visible" : "auto", padding: isMobile ? "12px" : "20px", display: "flex", flexDirection: "column", gap: isMobile ? 10 : 16 }}>
                      ...
                      return (
                        <div key={c.id} style={{ display: "flex", alignItems: "center", marginBottom: 8, height: 40 }}>
                          <div style={{ width: 90, flexShrink: 0, display: "flex", flexDirection: "column", gap: 1 }}>
                            <span style={{ fontSize: 12, fontWeight: 700, color: cBlocks.length > 0 ? "#ef4444" : "#a78bfa" }}>{c.icon} {c.id}</span>
                            <span style={{ fontSize: 9, color: "#374151" }}>{c.speciality}</span>
                          </div>
                          <div style={{ flex: 1, position: "relative", height: 34, background: "#0f0f1c", borderRadius: 8, overflow: "hidden" }}>
                            ...
                            {cBlocks.map(b => {
                              ...
                              return <div key={b.id} style={{ position: "absolute", top: 2, height: "calc(100% - 4px)", left: `${lp}%`, width: `${wp}%`, background: "#2b0a0a", border: "1px solid #991b1b", borderRadius: 6, display: "flex", alignItems: "center", paddingLeft: 6, overflow: "hidden", minWidth: 4, zIndex: 3 }}>
                                <span style={{ fontSize: 11, fontWeight: 600, color: "#ef4444", whiteSpace: "nowrap" }}>{wp > 8 ? `🚫${b.reason ? ` ${b.reason}` : ""}` : ""}</span>
                              </div>;
                            })}
          ...
          {view === "list" && (
            <div style={{ background: "#13131f", border: "1px solid #1a1a30", borderRadius: 14, overflow: "hidden", flex: 1 }}>
              <div style={{ padding: "12px 16px", borderBottom: "1px solid #1a1a30", fontWeight: 700, fontSize: 14 }}>Bookings — {fmtDate(date)}</div>
              <div style={{ overflow: "auto" }}>
                {allDay.length === 0 ? <div style={{ padding: 40, textAlign: "center", color: "#374151", fontSize: 14 }}>No bookings for this day</div> :
                  allDay.map((r, i) => {
                    const sc = SC[r.status];
          ...
          {view === "blocked" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ background: "#13131f", border: "1px solid #1a1a30", borderRadius: 14, padding: 16 }}>
                <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 16 }}>🚫 Block a Slot</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 16 }}>
                  <div>
                    <div style={{ fontSize: 12, color: "#475569", marginBottom: 6 }}>Consultant</div>
              ...
              <div style={{ background: "#13131f", border: "1px solid #1a1a30", borderRadius: 14, overflow: "hidden" }}>
                ...
                {blocked.length === 0 ? <div style={{ padding: 32, textAlign: "center", color: "#374151", fontSize: 14 }}>No blocked slots</div> :
                  blocked.map((b) => {
                      ...
                      <div key={b.id} style={{ padding: "14px 16px", borderBottom: "1px solid #1a1a30", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div>
                          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                            <span style={{ background: "#2b0a0a", border: "1px solid #991b1b", borderRadius: 6, padding: "2px 8px", fontSize: 13, fontWeight: 700, color: "#ef4444" }}>{b.consultant_id}</span>
